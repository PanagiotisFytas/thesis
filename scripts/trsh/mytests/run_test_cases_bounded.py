...
multicol = "\\multicolumn{1}{|c|}{Technique:} & \\multicolumn{" + str(max_B) + "}{c|}{Naive-BPOR} & \\multicolumn{" + str(max_B) + "}{c|}{Classic-BPOR} & \\multicolumn{"+ str(max_B) + "}{c|}{Source-BPOR} \\\\"
print(multicol)
print("\\hline")
...
for ts in programs:
	...
	for b in range(0,max_B):
		...
		print(' & ' + Van, end='')
	for b in range(0,max_B):
		SB = str(run_nidhugg(ts[0],"SB",b, df))
		print(' & ' + SB, end='')
	for b in range(0,max_B):
