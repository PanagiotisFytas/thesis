...
\subsection{Performance Evaluation - Why such an approach cannot succeed}
...
What is more, the rate with which new interleavings are planned for exploration, leads to schedulers not being sufficiently utilized throughout the complete execution of the algorithm. Specifically, we have noticed that the execution of the DPOR algorithm can be split into three consecutive phases. In the first phase, race detecting a single interleaving generates a large amount of interleavings that need to be explored. This means that, during this phase, the schedulers have enough work and are being kept utilized. During the second phase, race detecting an interleaving generates a relatively small amount of new interleavings that need to be explored. Generally, during this phase not enough work is being generated and the schedulers tend to stay idle. In the third phase,
